		...
		var buildTriples = function(){
			...
			if(this.getMapCreator() != null && this.getMapCreator().length > 0){
				if(isUrl(this.getMapCreator())){
					...
					if(this.getMapCreator().split(",").length > 1){
	

						   var arrayURIs = this.getMapCreator().split(",");

						    for (i = 0; i < arrayURIs.length; i++) {

								cAgentTriples = cAgentTriples + "\n" + paperMapUri + " <http://purl.org/dc/terms/creator> <" + arrayURIs[i] + ">" + tripleSeparator;
					...
					console.log("this.getMapCreator() -> "+this.getMapCreator());


				}else{
					var tmpMapCreator =  encodeURI(baseUri + "/" + this.getMapCreator());
					console.log("tmpMapCreator -> "+tmpMapCreator);
			...
			//Triples resulting from LOBID subject (GND)
